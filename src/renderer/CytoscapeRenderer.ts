    ...
    refreshDirectionalEdgeCurves(edges?: cytoscape.EdgeCollection): void {
        ...
        cy.batch(() => {
            targetEdges.forEach((edge: cytoscape.EdgeSingular) => {
                ...
                // 源主轴半尺寸:目标端肩长扣掉它,超宽父节点旁的层级边不再让目标控制点兜回父框内(丑陋回勾)。
                const sourceHalfMain = horizontal ? sd.w / 2 : sd.h / 2;
                // 源端肩短(0.3)+ 目标端肩中等(0.5):挺括扇形,离父快速转向、平顺切入子;对称 0.5/0.5 会回到软波浪。
                const cp = computeDirectionalEdgeControlPoints(s.x, s.y, t.x, t.y, direction, 0.3, 0.5, minTangentSource, minTangentTarget, targetHalfMain, sourceHalfMain);
