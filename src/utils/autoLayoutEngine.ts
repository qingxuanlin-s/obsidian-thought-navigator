...
export function computeAutoLayout(input: ComputeAutoLayoutInput): Record<string, Vec2> {
	...
	const isUniformPreset = Object.values(input.nodeLayoutPresets || {})
		.every((preset) => normalizeLayoutPreset(preset) === filePreset);
	if (isUniformPreset) {
		if (filePreset === 'bidirectional') return computeBidirectionalAutoLayout(input);
		if (filePreset === 'top-down') return computeTopDownAutoLayout(input);
		if (filePreset === 'radial') return computeRadialAutoLayout(input);
	}
...
export function computeTopDownAutoLayout(input: ComputeAutoLayoutInput): Record<string, Vec2> {
	const forwardGap = input.forwardGap ?? 150;
	const stackGap = input.stackGap ?? 56;
	const positions: Record<string, Vec2> = {};
	const directionCache = new Map<string, GrowthDirection | null>();
	const stackAxis: Vec2 = { x: 1, y: 0 };
	...
	const resolveTopDown = (nodeId: string, parentId: string): GrowthDirection => {
		const savedPos = input.nodePositions[nodeId];
		const parentPos = input.nodePositions[parentId] || input.nodes[parentId]?.position;
		if (savedPos && parentPos) {
			return quantizeToPool(savedPos.x - parentPos.x, savedPos.y - parentPos.y, ['S', 'N']);
		}
		const siblings = input.childrenById[parentId] || [];
		const index = Math.max(0, siblings.indexOf(nodeId));
		return index % 2 === 0 ? 'S' : 'N';
	};
	...
	const placeDirectedChildren = (layout: LayoutNode, cx: number, cy: number) => {
		...
		for (const child of layout.children) {
			...
			placeLayout(
				child,
				cx + dirVec.x * forward + stackAxis.x * stackOff,
				cy + dirVec.y * forward + stackAxis.y * stackOff
			);
			cursor += child.subtreeSpan + stackGap;
		}
	};
	...
	const placeGroupedChildren = (layout: LayoutNode, cx: number, cy: number) => {
		...
		for (const dir of ['S', 'N'] as GrowthDirection[]) {
			const children = groups.get(dir) || [];
			if (children.length === 0) continue;
			const dirVec = DIR_VECTORS[dir];
			const forward = projectSize(layout.size, dirVec) / 2 + forwardGap + avgProjection(children, dirVec) / 2;
...
export function computeRadialAutoLayout(input: ComputeAutoLayoutInput): Record<string, Vec2> {
	const forwardGap = input.forwardGap ?? 150;
	const stackGap = input.stackGap ?? 56;
	const positions: Record<string, Vec2> = {};
	const directionCache = new Map<string, GrowthDirection | null>();
	const radialPool: GrowthDirection[] = ['NE', 'SE', 'SW', 'NW'];
	const diagonalGap = forwardGap * DIAGONAL_GAP_FACTOR;
	...
	const getRadialStackAxis = (dir: GrowthDirection | null): Vec2 => {
		switch (dir) {
			case 'NE':
	...
	const resolveRadial = (nodeId: string, parentId: string): GrowthDirection => {
		const savedPos = input.nodePositions[nodeId];
		const parentPos = input.nodePositions[parentId] || input.nodes[parentId]?.position;
		if (savedPos && parentPos) {
			return quantizeToPool(savedPos.x - parentPos.x, savedPos.y - parentPos.y, radialPool);
		}
		const siblings = input.childrenById[parentId] || [];
		const index = Math.max(0, siblings.indexOf(nodeId));
		return radialPool[index % radialPool.length];
	};
	...
	const placeDirectedChildren = (layout: LayoutNode, cx: number, cy: number) => {
		...
		const dirVec = DIR_VECTORS[layout.dir];
		const axis = layout.stackAxis;
		const forward = projectSize(layout.size, dirVec) / 2 + diagonalGap + avgProjection(layout.children, dirVec) / 2;
		const total = layout.children.reduce((sum, child) => sum + child.subtreeSpan, 0)
			+ Math.max(0, layout.children.length - 1) * stackGap;
		let cursor = -total / 2;
	...
	const placeGroupedChildren = (layout: LayoutNode, cx: number, cy: number) => {
		...
		for (const dir of radialPool) {
			const children = groups.get(dir) || [];
			if (children.length === 0) continue;
			const dirVec = DIR_VECTORS[dir];
			const axis = getRadialStackAxis(dir);
			const forward = projectSize(layout.size, dirVec) / 2 + diagonalGap + avgProjection(children, dirVec) / 2;
			const total = children.reduce((sum, child) => sum + child.subtreeSpan, 0)
				+ Math.max(0, children.length - 1) * stackGap;
			let cursor = -total / 2;
